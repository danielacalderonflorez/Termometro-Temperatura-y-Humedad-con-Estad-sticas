# 1 "controlador_almacenamiento.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 295 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "controlador_almacenamiento.c" 2
# 1 "./controlador_almacenamiento.h" 1
...
# 6 "./controlador_almacenamiento.h" 2
# 25 "./controlador_almacenamiento.h"
...
# 2 "controlador_almacenamiento.c" 2
...
uint8_t Almacenamiento_TransferirSPI(uint8_t dato){
    ...
    for(i = 0; i < 8; i++){
        PORTBbits.RB1 = (dato & 0x80) ? 1 : 0;
        dato <<= 1;
        PORTBbits.RB3 = 1;
        _delay((unsigned long)((1)*(20000000/4000000.0)));
        resultado = (uint8_t)((resultado << 1) | (PORTBbits.RB2 ? 1 : 0));
        PORTBbits.RB3 = 0;
        _delay((unsigned long)((1)*(20000000/4000000.0)));
    }
    return resultado;
...
uint8_t Almacenamiento_Inicializar(void){
    ...
    for(reintentos = 0; reintentos < 200; reintentos++){
        if(Almacenamiento_EnviarComando(0x00, 0) == 0x01) break;
    }
    if(reintentos >= 200){ PORTBbits.RB0 = 1; return 0; }

    r = Almacenamiento_EnviarComando(0x08, 0x000001AA);
    if(r == 0x01){
        for(i = 0; i < 4; i++) Almacenamiento_TransferirSPI(0xFF);
    }

    for(reintentos = 0; reintentos < 500; reintentos++){
        Almacenamiento_EnviarComando(0x37, 0);
        if(Almacenamiento_EnviarComando(0x29, 0x40000000) == 0x00) break;
        _delay((unsigned long)((10)*(20000000/4000.0)));
    }
    if(reintentos >= 500){ PORTBbits.RB0 = 1; return 0; }

    if(Almacenamiento_EnviarComando(0x10, 512) != 0x00){ PORTBbits.RB0 = 1; return 0; }
...
void Almacenamiento_LeerDatos(uint32_t sector, uint16_t desplazamiento, uint8_t *buffer, uint8_t longitud){
    ...
    PORTBbits.RB0 = 0;
    if(Almacenamiento_EnviarComando(0x11, sector) != 0x00){ PORTBbits.RB0 = 1; return; }

    for(reintentos = 0; reintentos < 5000; reintentos++){
        r = Almacenamiento_TransferirSPI(0xFF);
        if(r == 0xFE) break;
    }
    if(reintentos >= 5000){ PORTBbits.RB0 = 1; return; }
...
void Almacenamiento_EscribirDatos(uint32_t sector, uint16_t desplazamiento, uint8_t *buffer, uint8_t longitud){
    ...
    respuesta = Almacenamiento_EnviarComando(0x18, sector);
    if(respuesta != 0x00){
        PORTBbits.RB0 = 1;
        Almacenamiento_TransferirSPI(0xFF);
        return;
