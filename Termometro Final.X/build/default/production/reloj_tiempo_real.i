...
# 2 "<built-in>" 2
# 1 "reloj_tiempo_real.c" 2
# 1 "./reloj_tiempo_real.h" 1
...
# 6 "./reloj_tiempo_real.h" 2
# 23 "./reloj_tiempo_real.h"
typedef struct {
    uint8_t segundos;
    uint8_t minutos;
    uint8_t horas;
...
# 2 "reloj_tiempo_real.c" 2



static __attribute__((inline)) void SDA_Liberar(void){ TRISCbits.TRISC4 = 1; }
static __attribute__((inline)) void SDA_ForzarBajo(void){ TRISCbits.TRISC4 = 0; PORTCbits.RC4 = 0; }
static __attribute__((inline)) void SCL_Liberar(void){ TRISCbits.TRISC3 = 1; }
static __attribute__((inline)) void SCL_ForzarBajo(void){ TRISCbits.TRISC3 = 0; PORTCbits.RC3 = 0; }
...
static void I2C_CondicionInicio(void){
    SDA_Liberar();
    SCL_Liberar(); _delay((unsigned long)((5)*(20000000/4000000.0)));
    SDA_ForzarBajo(); _delay((unsigned long)((5)*(20000000/4000000.0)));
    SCL_ForzarBajo();
}
...
static void I2C_CondicionParo(void){
    SDA_ForzarBajo(); _delay((unsigned long)((5)*(20000000/4000000.0)));
    SCL_Liberar(); _delay((unsigned long)((5)*(20000000/4000000.0)));
    SDA_Liberar(); _delay((unsigned long)((5)*(20000000/4000000.0)));
}
...
static uint8_t I2C_EscribirByte(uint8_t dato){
    ...
    SDA_Liberar();
    SCL_Liberar(); _delay((unsigned long)((5)*(20000000/4000000.0)));
    uint8_t confirmacion = (PORTCbits.RC4 == 0);
    SCL_ForzarBajo();
    return confirmacion;
}
...
uint8_t Reloj_VerificarConexion(void){
    I2C_CondicionInicio();
    uint8_t ok = I2C_EscribirByte(0x68<<1);
    I2C_CondicionParo();
    return ok;
}
...
uint8_t Reloj_ObtenerFechaHora(Reloj_FechaHora *fechahora){
    ...
    I2C_CondicionInicio();
    if(!I2C_EscribirByte(0x68<<1)) {
        I2C_CondicionParo();
        LimpiarFechaHora(fechahora);
        return 0;
    }
    if(!I2C_EscribirByte(0x00)) {
        I2C_CondicionParo();
        LimpiarFechaHora(fechahora);
        return 0;
    }
